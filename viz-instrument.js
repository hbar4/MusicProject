...
function anim4Base(vars, colorScheme) {
  drawGradient(colorScheme);
  for (var index = 0; index < 100; index = index + 1) {
    fill(colorScheme.primaryFill);
    stroke(colorScheme.primaryStroke);
    ellipse(vars["x"][index], vars["y"][index], 10);
    vars["x"][index] = vars["x"][index] + vars["xSpeed"][index];
    vars["y"][index] = vars["y"][index] + vars["ySpeed"][index];
  }
}

function anim4Intense(vars, colorScheme) {
  drawGradient(colorScheme);
  var newCenterX = random(width);
  var newCenterY = random(height);
  for (var index = 0; index < 100; index = index + 1) {
    vars["x"][index] = newCenterX;j
    vars["y"][index] = newCenterY;
    vars["xSpeed"][index] = random(-5, 5);
    vars["ySpeed"][index] = random(-5, 5);
    fill(colorScheme.primaryFill);
    stroke(colorScheme.highlight);
    ellipse(vars["x"][index], vars["y"][index], 10);
    vars["x"][index] = vars["x"][index] + vars["xSpeed"][index];
...
function anim5Base(vars, colorScheme) {
  drawGradient(colorScheme);
  var toDelete = [];
  var removed = false;
  for (var explosion = 0; explosion < vars["numExplosions"]; explosion++) {
    var stillDraw = false;
    for (var index = 0; index < 100; index = index + 1) {
      if ((vars["xs"][explosion][index] > 0 &&
          vars["xs"][explosion][index] < width &&
          vars["ys"][explosion][index] > 0 &&
          vars["ys"][explosion][index] < height)) {
        stillDraw = true;
      }
      fill(colorScheme.primaryFill);
      stroke(colorScheme.primaryStroke);
      ellipse(vars["xs"][explosion][index], vars["ys"][explosion][index], 10);
      vars["xs"][explosion][index] = vars["xs"][explosion][index] +
        vars["xSpeeds"][explosion][index];
      vars["ys"][explosion][index] = vars["ys"][explosion][index] +
        vars["ySpeeds"][explosion][index];
    }
    if (!stillDraw ||
      (millis() - vars["startTimes"][explosion] > 3000 &&
        vars["numExplosions"] == 10)) {
      if (!removed) {
    ...
    for (var i = toDelete.length - 1; i >= 0; i--) {
      vars["xs"].splice(toDelete[i], 1);
      vars["ys"].splice(toDelete[i], 1);
      vars["xSpeeds"].splice(toDelete[i], 1);
      vars["ySpeeds"].splice(toDelete[i], 1);
      vars["startTimes"].splice(toDelete[i], 1);
      vars["numExplosions"]--;
    }
  }
}

function anim5Intense(vars, colorScheme) {
  drawGradient(colorScheme);
  var newCenterX = random(width);
  var newCenterY = random(height);
  var newExplosionXs = [];
  var newExplosionYs = [];
  var newExplosionXSpeeds = [];
  var newExplosionYSpeeds = [];
  var toDelete = [];
  if (vars["numExplosions"] < 10) {
    for (var index = 0; index < 100; index++) {
      newExplosionXs[index] = newCenterX;
      newExplosionYs[index] = newCenterY;
      newExplosionXSpeeds[index] = random(-5, 5);
      newExplosionYSpeeds[index] = random(-5, 5);
    }
    vars["numExplosions"]++;
    vars["xs"].push(newExplosionXs);
    vars["ys"].push(newExplosionYs);
    vars["xSpeeds"].push(newExplosionXSpeeds);
    vars["ySpeeds"].push(newExplosionYSpeeds);
    vars["startTimes"].push(millis());
  }
  for (var explosion = 0; explosion < vars["numExplosions"]; explosion++) {
    var stillDraw = false;
    for (var index = 0; index < 100; index = index + 1) {
      if ((vars["xs"][explosion][index] > 0 &&
          ...
          vars["ys"][explosion][index] > 0 &&
          vars["ys"][explosion][index] < height) ||
        millis() - vars["startTimes"][explosion] > 2000) {
        stillDraw = true;
      }
      fill(colorScheme.primaryFill);
      stroke(colorScheme.primaryStroke);
...
var anim4 = new AnimationScheme(anim4Base, anim4Intense, vars4);
var anim5 = new AnimationScheme(anim5Base, anim5Intense, vars5);
